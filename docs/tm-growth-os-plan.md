...
2. ✅ **PostFlow API surface — RESOLVED 2026-07-27. Yes, and it covers the full handoff.** Verified against `postflow.app/docs` + `docs/llms.txt`. Base `https://api.postflow.app/v1`, bearer-token auth, media at `https://media.postflow.app/api/v1`. Confirmed: create + schedule posts, **explicit draft set/unset** (the design's "drafts land in PostFlow" works as drawn), list/filter/retrieve post groups with full data (up to 200/request), **per-post and per-group analytics** incl. per-story breakdowns, media upload (direct + URL), activity logging per post group. **Module E is unblocked on PostFlow** and PostFlow is not being replaced. **Rate limits confirmed: 60 requests/minute per authenticated user**, with `X-RateLimit-Limit` / `-Remaining` / `-Reset` headers on every response — comfortable for daily batch collection provided the collector uses the bulk endpoints (200/request) rather than per-post loops, and honors the headers. Note the limit is per *token*, so one agency token shares a single bucket across all clients. Left to confirm at build time, neither of which changes scope: whether API access is plan-tier gated · whether post details expose a published **permalink** or native platform post ID (the join key between publishing and platform analytics). Needs a token generated from account settings → vault as `postflow`.
...
8b-ii. **WordPress clients — use a dedicated Editor account, never a personal admin login.** The first site connected (Alpha Zeta, 2026-07-28) authenticated with an application password on the owner's **administrator** account, carrying `edit_themes`, `edit_plugins`, `edit_files`, `edit_users` and `manage_options` — full control of a live client site, for an integration that only edits page content. Create a `growth-os` user with the **Editor** role and generate the application password from that account. Editor covers `edit_posts` / `edit_pages` / `publish_pages`, which is everything the adapter does, and cannot touch themes, plugins, files or users. It also decouples the integration from any one person's login. Verify with `/api/ops/wordpress-check` → `can_write: true`.

8b-iii. **Install `wordpress-mu-plugins/tm-growth-os-seo-rest.php`** into the site's `wp-content/mu-plugins/`. WordPress does not expose arbitrary post meta over REST, and Rank Math stores its SEO fields as plain post meta without registering them — so SEO title and meta description are invisible to the API even though the plugin is installed and working. Title, content and excerpt changes work without it; SEO fields do not. Verify with `/api/ops/wordpress-check` → `seo_fields_writable: true`. Every TM non-eCommerce build is WordPress, so this applies to all of them.
