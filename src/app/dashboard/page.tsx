// app/dashboard/page.tsx — server component, reads client_dashboard view.
// Styling matches the TM design system: stone surface, serif metric
// numerals (StatCard pattern), performance-green accents, quiet chrome.
...
export default async function Dashboard() {
  ...
  const { data } = await db.from("client_dashboard").select("*").order("name");
  const rows = (data ?? []) as Row[];

  return (
    <main style={{
      fontFamily: "var(--font-body)", background: "var(--bg)",
      minHeight: "100vh", padding: "48px 24px", color: "var(--fg1)",
    }}>
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        ...
        <h1 style={{
          ...
          fontSize: 56, letterSpacing: "-0.01em", margin: "10px 0 40px",
        }}>
              ...
              <header style={{
                ...
                {r.tier && (
                  <span style={{
                    ...
                    color: "var(--tm-performance-green)",
                  }}>{r.tier}</span>
                )}
              </header>
              <div style={{
